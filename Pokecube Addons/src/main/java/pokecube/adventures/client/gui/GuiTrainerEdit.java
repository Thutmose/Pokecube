...
public class GuiTrainerEdit extends GuiScreen
{
	...
	public void initGui()
	{
		...
		textfieldPokedexNb0 = new GuiTextField(0, fontRendererObj, width / 2 - 70 + xOffset, height / 4 + 60 + yOffset, 30,
				10);
		textfieldLevel0 = new GuiTextField(0, fontRendererObj, width / 2 - 15 + xOffset, height / 4 + 60 + yOffset, 30,
				10);
		textfieldPokedexNb1 = new GuiTextField(0, fontRendererObj, width / 2 - 70 + xOffset, height / 4 + 80 + yOffset, 30,
				10);
	...
	public void drawScreen(int i, int j, float f)
	{
		...
		this.fontRendererObj.drawString(info, x1 + 90 - l / 2, y1 - 100, 0xffffff);

		int num = 0;
//		if (trainer.getEquipmentInSlot(1) != null) RenderItem.getInstance().renderItemIntoGUI(fontRendererObj,
//				mc.getTextureManager(), trainer.getEquipmentInSlot(1), x1 + 50, y1 - 135);//TODO find out how to render item into gui
	...
	private void renderMob(PokedexEntry mob)
	{
		try
		{
			...
			entity.onGround = ((IPokemob) entity).getType1() != flying
					&& ((IPokemob) entity).getType2() != flying;
			int i = 15728880;
			int j1 = i % 65536;
			int k1 = i / 65536;
