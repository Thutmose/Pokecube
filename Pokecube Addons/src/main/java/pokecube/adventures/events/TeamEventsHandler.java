...
public class TeamEventsHandler 
{
    ...
    public void placeEvent(PlayerInteractEvent evt)
    {
    	if(evt.action==Action.RIGHT_CLICK_BLOCK)
    	{
        	...
        	if(block!=null && evt.entityPlayer.getHeldItem()!=null && evt.entityPlayer.getHeldItem().getItem() instanceof IPokecube)
        	{
        	...
        	if(!b && evt.entityPlayer.getHeldItem()==null)
        		return;
        	...
        	if(!team.equals(owner))
        	{
        		...
        		{
	        		evt.entityPlayer.addChatMessage(new ChatComponentText("You must be a member of Team "+owner+" to do that."));
    ...
    public void BreakBlock(BreakEvent evt)
    {
    	...
    	if(player!=null && player.getTeam()!=null)//TODO interface with forge permissions API here as well
    	{
    		...
    		if(!player.worldObj.isRemote)
    		{
				...
				if(userentry != null || !FMLCommonHandler.instance().getMinecraftServerInstance().isDedicatedServer())
				{
					return;
				}
    		}
            if(TeamManager.getInstance().isOwned(c) && !TeamManager.getInstance().isTeamLand(c, player.getTeam().getRegisteredName()))
            {
            	player.addChatMessage(new ChatComponentText("You may not remove blocks from land owned by Team "+TeamManager.getInstance().getLandOwner(c)));
            	evt.setCanceled(true);
            	return;
            }
