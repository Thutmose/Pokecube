...
public class TeamManager {
	...
	{
		if(landCounts.containsKey(team))
			return landCounts.get(team);
		return 0;
	}
	...
	public void addTeamLand(String team, ChunkCoordinate land)
	{
		...
		if(landMap.containsKey(land))
		{
			...
			if(landCounts.containsKey(old))
			{
				...
				if(landCounts.get(old)<=0)
					landCounts.remove(old);
			}
		}
	...
	{
		if(landMap.containsKey(chunk))
		{
			return landMap.get(chunk).equals(team);
		}
		return false;
	}
