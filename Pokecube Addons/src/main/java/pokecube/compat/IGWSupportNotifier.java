...
public class IGWSupportNotifier{
    ...
    private class ThreadDownloadIGW extends Thread{
        ...
        public void run(){
            ...
            try {
                ...
                e.printStackTrace();
                if(Minecraft.getMinecraft().thePlayer != null) Minecraft.getMinecraft().thePlayer.addTextComponentMessage(new TextComponentString(TextFormatting.RED + "Failed to download"));
                try {
                    finalize();
                } catch(Throwable e1) {
                    e1.printStackTrace();
    ...
    public void onPlayerJoin(TickEvent.PlayerTickEvent event){
        if(event.player.worldObj.isRemote && event.player == FMLClientHandler.instance().getClientPlayerEntity()) {
            event.player.addTextComponentMessage(ITextComponent.Serializer.jsonToComponent("[\"" + TextFormatting.GOLD + "The mod " + supportingMod + " is supporting In-Game Wiki mod. " + TextFormatting.GOLD + "However, In-Game Wiki isn't installed! " + "[\"," + "{\"text\":\"Download Latest\",\"color\":\"green\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/igwmod_download\"}}," + "\"]\"]"));
