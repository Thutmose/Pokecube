...
public class AchievementHatch extends Achievement
{
    ...
    public AchievementHatch(PokedexEntry entry, int column, int row, Item item, Achievement parent)
    {
        super("achievement.pokecube." + entry.getName() + ".hatch", entry.getName(), column, row, item, parent);
        this.entry = entry;
    }
    ...
    {
        return I18n.format("achievement.pokecube.hatch", I18n.format(getPokemobTranslatedName()));
    }
    ...
    {
        ITextComponent iTextComponent = new TextComponentTranslation("achievement.pokecube.hatch",
