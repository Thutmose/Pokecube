...
/**
 * Follow Owner AI.  The pokemon should follow owner if it is not sitting, not staying, not guarding and not attacking.
 * 
 * Currently the teleporting is disabled, due to the improvements to the pathfinder, and the ability to recall remotely with r.
 * 
 * @author Patrick
 *
 */
	...
	IPokemob pokemob;
	   ...
	   private IEntityOwnable pet;
		...
		public boolean shouldExecute()
	    {
	        ...
	        else if(pokemob!=null&&(pokemob.getPokemonAIState(IPokemob.GUARDING)||pokemob.getPokemonAIState(IPokemob.STAYING)))
	        {
	        	return false;
	        }
	    ...
	    {
	        return !this.petPathfinder.noPath() && !pokemob.getPokemonAIState(IPokemob.SITTING) && this.thePet.getDistanceSqToEntity(this.theOwner) > this.maxDist * this.maxDist;
	    }
...
//	        this.thePet.getNavigator().setAvoidsWater(this.field_75344_i);
		...
		public void updateTask()
	    {
	    	...
	    	else if(!this.petPathfinder.noPath() && petPathfinder.getPath().getCurrentPathIndex()<petPathfinder.getPath().getCurrentPathLength()-3)
	    	{
	    		double x,y,z;
	    		x = petPathfinder.getPath().getPathPointFromIndex(petPathfinder.getPath().getCurrentPathIndex()+1).xCoord + 0.5;
	    		y = petPathfinder.getPath().getPathPointFromIndex(petPathfinder.getPath().getCurrentPathIndex()+1).yCoord + 0.5;
	    		z = petPathfinder.getPath().getPathPointFromIndex(petPathfinder.getPath().getCurrentPathIndex()+1).zCoord + 0.5;
