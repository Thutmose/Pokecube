 ...
 * nearby pokemon of the same species trying to protect the pokemon, resulting in
 * horde like behavior.
 * @author Patrick
 *
 */
public class PokemobAIHurt extends EntityAIBase {
    ...
    protected boolean isSuitableTarget(EntityLivingBase p_75296_1_, boolean p_75296_2_)
    {
        ...
        {
            if (this.taskOwner instanceof IEntityOwnable && StringUtils.isNotEmpty(((IEntityOwnable)this.taskOwner).getOwnerId()))
            {
            ...
            if (!this.taskOwner.isWithinHomeDistanceFromPosition(new BlockPos(MathHelper.floor_double(p_75296_1_.posX), MathHelper.floor_double(p_75296_1_.posY), MathHelper.floor_double(p_75296_1_.posZ))))
            {
                return false;
            }
