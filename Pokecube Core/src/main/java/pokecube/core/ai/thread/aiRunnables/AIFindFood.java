...
public class AIFindFood extends AIBase
{
	...
	public void run()
	{
		if (foodLoc.isEmpty())
		{
			...
			{
				if (entity.worldObj.provider.isDaytime() && v.canSeeSky(world))//TODO re-do this for threadsafe
				{
			...
			{
//				List<Entity> list = world.getEntitiesWithinAABB(EntityFishHook.class, AxisAlignedBB
//						.getBoundingBox(entity.posX, entity.posY, entity.posZ, entity.posX + 1.0D, entity.posY + 1.0D, entity.posZ + 1.0D)
...
//						System.out.println("Found a bait " + pokemob.getPokemonDisplayName());
	//
//						PathEntity path =entity.getNavigator().getPathToEntityLiving(nearest);
