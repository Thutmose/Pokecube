...
public class AIFindTarget extends AIBase implements IAICombat
{
	...
	public void run()
	{
        if (entity.getAttackTarget() == null && !pokemob.getPokemonAIState(IPokemob.STAYING) &&pokemob.getPokemonAIState(IPokemob.TAMED) && !mod_Pokecube.isOnClientSide())
        {
        	List<Object> list = getEntitiesWithinDistance(entity, EntityLivingBase.class, 16);
            if (!list.isEmpty() && pokemob.getPokemonOwner()!=null)
            {
                for (int j = 0; j < list.size(); j++)
                {
                    ...
                    if (entity instanceof EntityCreature
                            && ((EntityCreature) entity).getAttackTarget() != null
        ...
        if(entity.getAttackTarget() == null && !pokemob.getPokemonAIState(IPokemob.SITTING) && hungryMob.isCarnivore() && pokemob.getPokemonAIState(IPokemob.HUNTING))
        {
        	List<Object> list = getEntitiesWithinDistance(entity, EntityLivingBase.class, 16);
            if (!list.isEmpty())
            {
         ...
         if(pokemob.getPokemonAIState(IPokemob.GUARDING))
         {
 	    	...
 	    	pokemobs = getEntitiesWithinDistance(entity, EntityLivingBase.class, 32);
	...
	List<Object> getEntitiesWithinDistance(Entity source, Class<EntityLivingBase> targetClass, int distance)
	{
