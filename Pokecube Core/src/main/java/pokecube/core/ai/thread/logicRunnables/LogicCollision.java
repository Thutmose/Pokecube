...
public class LogicCollision extends LogicBase
{
	...
	/**
	 * Index 0 = reading/writing from entity, 1 = setting/reading diffs, 2 = ready for pokemob to update
	 */
...
//		lock[1] = true;
		...
		Vector3 here = Vector3.getNewVectorFromPool().set(e);
		double x,y,z;
		...
		for(String s: boxes.keySet())
		{
			...
			Vector3 v;
			diffs.set(v = box.doTileCollision(world, (Entity) e, pos, diffs));
		...
		diffsArr[1] = y;
		diffsArr[2] = z;
//		System.out.println(Arrays.toString(prevDiffs)+" "+diffs+Arrays.toString(diffsArr));
		diffs.freeVectorFromPool();
		
		//System.out.println(Arrays.toString(diffsArr)+" "+Arrays.toString(prevDiffs));
