...
{
	public static final PropertyEnum	VARIANT4	= PropertyEnum.create("variant", BlockBerryWood.EnumType.class,
			new Predicate()
			{
				...
				{
					return this.apply((BlockBerryWood.EnumType) p_apply_1_);
				}
			});
	public static final PropertyEnum	VARIANT0	= PropertyEnum.create("variant", BlockBerryWood.EnumType.class,
			new Predicate()
			{
				public boolean apply(BlockBerryWood.EnumType type)
				{
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	{
		if (!worldIn.isRemote)
		{
			...
			if(canGrow)
			{
				 BlockBerryFruit fruit = (BlockBerryFruit) ((BlockBerryWood.EnumType) state.getValue(shift == 0 ? VARIANT0 : VARIANT4)).getBerryFruit();
	 ...
	 * returns 4 blocks) */
	public void getSubBlocks(Item itemIn, CreativeTabs par2CreativeTabs, List list)
	{
		if (shift == 0)
		{
	...
	public List<ItemStack> onSheared(ItemStack item, net.minecraft.world.IBlockAccess world, BlockPos pos, int fortune)
	{
		...
		return new java.util.ArrayList(java.util.Arrays.asList(new ItemStack(this, 1,
				((BlockPlanks.EnumType) state.getValue((shift == 0 ? VARIANT0 : VARIANT4))).getMetadata() - shift)));
	}
	...
	public IBlockState getStateForTree(String berryName)
	{
		...
		if(type!=null)
		{
			...
			if(num >= 0 && num < 4)
			{
				return getStateFromMeta(num + shift).withProperty(CHECK_DECAY, Boolean.FALSE).withProperty(DECAYABLE, Boolean.TRUE);
			}
		}
		return null;
	}
