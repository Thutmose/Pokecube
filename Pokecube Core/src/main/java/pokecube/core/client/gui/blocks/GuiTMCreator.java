...
public class GuiTMCreator extends GuiContainer
{
    ...
    protected void actionPerformed(GuiButton guibutton)
    {
        ...
        if(guibutton.id==4&&!moves.isEmpty())
        {
        	...
        	MessageServer packet = PCPacketHandler.makeServerPacket(PokecubePacketHandler.CHANNEL_ID_TradingTable, message.getBytes());
    ...
    protected void keyTyped(char par1, int par2)
    {
        ...
        for(String s: moves)
        {
        	...
        	boolean nameMatch = MovesUtils.getTranslatedMove(s.trim()).toLowerCase().contains(textFieldSearch.getText().toLowerCase());
        	boolean typeMatch = false;
        	if(!nameMatch)
        	{
	        	for(PokeType t: PokeType.values())
	        	{
	        		if(move.getType() == t && PokeType.getTranslatedName(t).toLowerCase().contains(textFieldSearch.getText().toLowerCase()))
	        		{
	        			typeMatch = true;
	        			break;
	        		}
