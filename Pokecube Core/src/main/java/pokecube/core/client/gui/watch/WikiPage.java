...
public class WikiPage extends ListPage
{
    ...
    private void setList()
    {
        ...
        for (int i = 0; i < bookPages.tagCount(); i++)
        {
            ...
            List<ITextComponent> list = GuiUtilRenderComponents.splitText(page, 120, fontRendererObj, true, true);
            for (int j = 0; j < list.size(); j++)
            {
                ...
                entries.add(new WikiLine((watch.height - 160) / 2 + 25, (watch.height - 160) / 2 + 132, fontRendererObj,
                        line, i).setClickListner(listener));
            }
        }
        list = new ScrollGui(mc, 135, height, fontRendererObj.FONT_HEIGHT + 2, offsetX, offsetY, entries);
    }
