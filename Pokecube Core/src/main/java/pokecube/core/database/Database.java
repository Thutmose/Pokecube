...
public class Database implements IMoveConstants {
	...
	public static String[] defaultStarts = new String[]{
		"Thutmose:mew;Sshiny`Micebeam:shuckle;Sshiny`Mrollout`Macupressure`Mrest`Mgyroball:null",
		  "Legnak:beldum;R0:null",
		  "Oracion:mew:Sshiny:null",
		  "Manchou:mew:Sshiny:null",
		  "cflame13:mew:Sshiny:null",
	...
	public static void postInit()
	{
		...
		for(PokedexEntry p: Database.data.values())
		{
			try {
		...
		//Reloads all the sizes for any formes which were missed on the first pass due to lack of pokedex entries
		updateSizes();
		toRemove.clear();
	...
	public static SpawnData getSpawnData(int nb) {
		if (data.containsKey(nb))
			return data.get(nb).getSpawnData();
		return null;
	}
	...
	public static boolean compare(String a, String b)
	{
		...
		ret = a.toLowerCase().replaceAll("(\\W)", "").equals(
				b.toLowerCase().replaceAll("(\\W)", ""));
		
		return ret;
	}
	
	public static List<String> getLevelUpMoves(int nb, int level, int oldLevel) {
		return entryExists(nb) ? getEntry(nb).getMovesForLevel(level, oldLevel)
				: null;
	}
	...
	private static void loadSpawns() {
		...
		if(PokecubeMod.debug)
			System.out.println("loaded Standard Spawns ");
		...
		for(ArrayList<String> s: rows)
		{
			...
			if(e!=null)
			{
				...
				if(!justSize)
				{
					...
					if(forme.equals(""))
						System.err.println("Missing name in Stats file for a form of "+name);
					else
						name = forme.trim();
					
					if(getType(first)==getType(second))
						System.err.println("Error in Types for form "+name+" of "+e.name+", Listed as '"+first+"' and '"+second+"'");
					
					e.addStats(name, Integer.parseInt(s.get(2)), Integer.parseInt(s.get(3)), Integer.parseInt(s.get(4)), 
							Integer.parseInt(s.get(5)), Integer.parseInt(s.get(6)), Integer.parseInt(s.get(7)), catchRate, getType(first), getType(second));
	...
	private static void loadEVXP(String s2)
	{
		...
		for(ArrayList<String> s: rows)
		{
			...
			if(e!=null)
			{
				...
				if(s.get(1).equals(""))
					System.err.println("Missing name in EV file for a form of "+name);
				else
					name = s.get(1).trim();
				...
				if(s.size()>18)
				{
					...
					if(args.length>1 && !args[1].isEmpty())
						e.shouldFly= Boolean.parseBoolean(args[1]);
					if(args.length>2 && !args[2].isEmpty())
						e.shouldDive= Boolean.parseBoolean(args[2]);
					if(args.length>3 && !args[3].isEmpty())
						e.shouldDive= Boolean.parseBoolean(args[3]);
				}
				if(s.size()>19)
				{
				...
				e.addEVXP(name, Integer.parseInt(s.get(3)), Integer.parseInt(s.get(4)), Integer.parseInt(s.get(5)), 
						Integer.parseInt(s.get(6)), Integer.parseInt(s.get(7)), Integer.parseInt(s.get(8)), xpGain, evolutionMode, sexRatio);
	...
	private static void loadSpawns(String file)
	{
		...
		for(ArrayList<String> s: rows)
		{
			...
			if (!entryExists(name)) {
				...
				System.err.println("'"
								+ name
								+ "' Does not exist in the Database, or is spelt incorrectly");
				continue;
			}
			...
			/**
			 * Column 0: Name
			 * Column 1: cases (day/night/fossil/starter/legend/water+/water)
			 * Column 2 any biomes
			 * Column 3 all biomes
			 * Column 4 no biomes
			 */
				...
				for(String an: any)
				{
					...
					for(int i = 0; i<vals.length-1; i++)
					{
						...
						else
						{
							...
							{
								//System.out.println("Error with spawndata for "+name);
							}
						}
			...
			if(entry.isValid(BiomeType.CAVE.getType()))
				entry.types[SpawnData.CAVE] = true;
			if(entry.isValid(BiomeType.VILLAGE.getType()))
				entry.types[SpawnData.VILLAGE] = true;
			if(entry.isValid(BiomeType.INDUSTRIAL.getType()))
				entry.types[SpawnData.INDUSTRIAL] = true;
			
			
			dbe.setSpawnData(entry);
			if(!spawnables.contains(dbe))
				spawnables.add(dbe);
	...
	public static void loadMoves(String file)
	{
		...
		for(ArrayList<String> s: rows)
		{
			if(s==null||s.size()<7||s.get(0).equalsIgnoreCase("number"))
				continue;
			 ...
			 if(cat.contains("spec")||cat.contains("status"))
           ...
           int[] old = modifiers.clone();
	       	...
	       	if(changes.length>1)
	       	{
	       		...
	       		move.selfDamageType = cond.contains("miss")? MoveEntry.MISS:
	       			cond.contains("hp")?MoveEntry.RELATIVEHP:
	       				MoveEntry.DAMAGEDEALT;
	       	}
	...
	public static String convertMoveName(String moveNameFromBulbapedia) {
		...
		String ret = "";
		String name = moveNameFromBulbapedia.trim().toLowerCase()
				.replaceAll("[^\\w\\s ]", "");
