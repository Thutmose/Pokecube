...
public class PokedexEntry {
	public static class EvolutionData {
		...
		private void parse(String data) {
			...
			for (String s : parts) {
				...
				if (arg1.equals("L")) {
					...
					if (arg2.equalsIgnoreCase("day"))
						dayOnly = true;
					if (arg2.equalsIgnoreCase("night"))
						nightOnly = true;
					...
					if (arg2.equalsIgnoreCase("male"))
						gender = 1;
					if (arg2.equalsIgnoreCase("female"))
						gender = 2;
		...
		public boolean shouldEvolve(IPokemob mob, ItemStack mobs) {
			...
			if (mob instanceof EntityLiving && ((EntityLiving) mob).getHeldItemMainhand() != null
					&& ((EntityLiving) mob).getHeldItemMainhand().isItemEqual(PokecubeItems.getStack("everstone"))) {
				return false;
			}
			if (mobs != null && mobs.isItemEqual(PokecubeItems.getStack("everstone"))) {
				return false;
			}
			ret = ret && correctItem;
			...
			if (ret && !biome.isEmpty()) {
				...
				if (type == -1) {
					for (ResourceLocation key : BiomeGenBase.biomeRegistry.getKeys()) {
						...
						if (b != null)
							if (b.getBiomeName().equalsIgnoreCase(biome))
								type = BiomeGenBase.getIdForBiome(b);
					}
				}
	...
	public static class InteractionLogic {
		...
		private static ItemStack parseStack(String info) {
			...
			if (item != null)
				ret = new ItemStack(item, 1, damage);
			else
				new NullPointerException("Errored Item for " + info);
			return ret;
		}
		...
		boolean processInteract(EntityPlayer player player, IPokemob pokemob, boolean doInteract) {
	...
	public static class SpawnData {
		public static class TypeEntry {
			...
			public boolean isValid(int biome) {
				if (types.contains(BiomeType.ALL.getType()))
					return true;
		...
		public final boolean[] types = new boolean[] { false, false, false, false, false, false, false, false, false,
		...
		public int getMax(int biome) {
			if (biomes.containsKey(biome)) {
				return biomes.get(biome).groupMax;
			}
			if (biomes.containsKey(BiomeType.ALL.getType()))
				return biomes.get(BiomeType.ALL.getType()).groupMax;
			return 4;
		}
		...
		public int getMin(int biome) {
			...
			if (biomes.containsKey(biome)) {
				return biomes.get(biome).groupMin;
			}
			if (biomes.containsKey(BiomeType.ALL.getType()))
				return biomes.get(BiomeType.ALL.getType()).groupMin;
		...
		public float getWeight(int biome) {
			if (biomes.containsKey(biome)) {
				return biomes.get(biome).weight;
			}
			if (biomes.containsKey(BiomeType.ALL.getType()))
				return biomes.get(BiomeType.ALL.getType()).weight;
		...
		public boolean isValid(int biome, int subbiome) {
			...
			if (biomes.containsKey(biome)) {
				if (biomes.containsKey(subbiome))
					return true;
				for (TypeEntry t : anyTypes) {
					if (t.isValid(subbiome)) {
						return true;
					}
				}
				for (TypeEntry ta : allTypes) {
					if (ta.isValid(subbiome)) {
						return true;
					}
				}
				return false;
		...
		private boolean isValidInit(int b) {
			if (b < BiomeGenBase.getBiomeGenArray().length)
				for (Type t : noTypes) {
					if (BiomeDatabase.contains(BiomeGenBase.getBiomeGenArray()[b], t))
						return false;
				}
		...
		public void postInit() {
			...
			biomes: for (BiomeType b : BiomeType.values()) {
				if (b == null || b == NONE)
					continue;
	...
	protected void addItem(String toParse, Map<ItemStack, Integer> toAddTo) {
		...
		int chance = 100;
		if (drop.length > 3)
			chance = Integer.parseInt(drop[3]);
	...
	private void addRelation(PokedexEntry toAdd) {
		if (!related.contains(toAdd) && toAdd != null)
			related.add(toAdd);
	}
	...
	public boolean canEvolve(int level, ItemStack stack) {
		for (EvolutionData d : evolutions) {
			...
			if (d.level >= 0 && level >= d.level && itemCheck)
				return true;
		}
	...
	public void copyToForm(PokedexEntry e) {
		...
		if (e.possibleMoves == null)
			e.possibleMoves = possibleMoves;
		if (e.lvlUpMoves == null)
		...
		if (e.length == -1)
			e.length = length;
		if (e.childNumbers.isEmpty())
			e.childNumbers = childNumbers;
		if (e.species == null)
			e.species = species;
		if (e.mobType == null)
			e.mobType = mobType;
		if (e.catchRate == -1)
			e.catchRate = catchRate;
		if (e.mass == -1)
			e.mass = mass;
		if (e.foodDrop == null)
			e.foodDrop = foodDrop;
		if (e.commonDrops.isEmpty())
			e.commonDrops = commonDrops;
		if (e.rareDrops.isEmpty())
			e.rareDrops = rareDrops;
	...
	public Ability getAbility(int number) {
		if (number < abilities.size()) {
			return AbilityManager.getAbility(abilities.get(number));
		}
		return null;
	}
	...
	public PokedexEntry getEvo(IPokemob pokemob) {
		for (Entry<PokedexEntry, MegaRule> e : megaRules.entrySet()) {
			...
			if (rule.shouldMegaEvolve(pokemob))
				return entry;
		}
		return null;
	}
	...
	public Ability getHiddenAbility(IPokemob pokemob) {
		if (abilitiesHidden.isEmpty())
			return null;
		else if (abilitiesHidden.size() == 1)
			return AbilityManager.getAbility(abilitiesHidden.get(0));
		else if (abilitiesHidden.size() == 1)
			return pokemob.getSexe() == IPokemob.MALE ? AbilityManager.getAbility(abilitiesHidden.get(1))
					: AbilityManager.getAbility(abilitiesHidden.get(1));
		return null;
	...
	public List<String> getMovesForLevel(int level, int oldLevel) {
		...
		if (oldLevel == 0)
			return getMovesForLevel(level);

		for (int i = oldLevel; i < level; i++) {
			if (lvlUpMoves.get(i + 1) == null)
				continue;
	...
	public ItemStack getRandomCommonDrop(int looting) {
		...
		if (ret == null || rand.nextInt(100) > chance)
			return null;
		ret = ret.copy();
	...
	protected void initPrey() {
		...
		for (String s : food)
			foodList.add(s);
		poke: for (PokedexEntry e : Database.data.values()) {
			if (e.species != null)
				for (String s : e.species) {
	 ...
	 * @return
	 */
	public boolean processInteract(EntityPlayer player player, IPokemob pokemob, boolean doInteract) {
