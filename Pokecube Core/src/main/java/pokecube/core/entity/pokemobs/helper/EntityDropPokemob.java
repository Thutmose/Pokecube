...
public abstract class EntityDropPokemob extends EntityMovesPokemob {
    ...
    protected void dropFewItems(boolean flag, int nb)
    {
		...
		if (!getPokemonAIState(IPokemob.TAMED) && food != null) {
			if (isBurning()) {
				if (food.getItem() == Items.chicken)
					dropItem(Items.cooked_chicken, j);
				else if (food.getItem() == Items.potato)
					dropItem(Items.baked_potato, j);
				else if (food.getItem() == Items.beef)
					dropItem(Items.cooked_beef, j);
				else if (food.getItem() == Items.fish)
					dropItem(Items.cooked_fish, j);
				else if (food.getItem() == Items.porkchop)
					dropItem(Items.cooked_porkchop, j);
				else
		...
		if(food==null&&this.getDropItem()!=null)
			food = new ItemStack(this.getDropItem());
		if (food!=null) {
