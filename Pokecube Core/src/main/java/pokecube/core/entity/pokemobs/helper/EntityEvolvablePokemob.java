 ...
 //   private int evolution;
    ...
    public IPokemob evolve(boolean showAnimation, ItemStack stack) {
		if (this.getPokedexEntry().canEvolve() && !isDead) 
		{
			...
			if(evolution==null||evolution.isEmpty())
			{
				for(EvolutionData d: getPokedexEntry().getEvolutions())
				{
					if(d.shouldEvolve(this, stack))
					{
						...
						if(!d.shouldEvolve(this, null) && stack == getHeldItem())
							neededItem = true;
						break;
					}
				}
			...
			if(evol!=null)
			{
				IPokemob evo = megaEvolve(evol.getName());
				if(neededItem)
				{
				...
				if(evo!=null)
				{
			    	...
			    	EvolveEvent evt = new EvolveEvent(evo);
			    		...
			    		if(evo.getPokedexEntry()==Database.getEntry("ninjask"))
			    		{
			    	    	...
			    	    	if(hasCube && hasSpace)
			    	    	{
			    	    		...
			    	    		if(pokemon!=null)
			    	    		{
						    		...
						    		cube.stackSize--;
						    		if(cube.stackSize<=0)
						    			inv.setInventorySlotContents(m, null);
    ...
    {
    	if(itemstack!=null && itemstack.isItemEqual(PokecubeItems.getStack("Everstone")))
    		return false;
    	
    	if (this.getPokedexEntry().canEvolve()
				&& !mod_Pokecube.isOnClientSide()) {
			if(evolution.isEmpty())
			{
				for (EvolutionData d : getPokedexEntry().getEvolutions()) {
					if (d.shouldEvolve(this, itemstack)) {
						return true;
					}
				}
			}
    ...
    @Override
    /**
     * These have been deprecated, now override evolve() to make a custom evolution occur.
     * @param evolution
     */
