...
public abstract class EntityHungryPokemob extends EntityAiPokemob {
    ...
    public void onLivingUpdate()
    {
    	...
    	if(hungerTime>hurtTime && !worldObj.isRemote 
    	...
    	boolean ownedSleepCheck = getPokemonAIState(IPokemob.TAMED) && !(getPokemonAIState((byte) (STAYING))||getPokemonAIState((byte) (SITTING)));
    ...
    public void eat(Entity e)
    {
    	...
    	if(e instanceof EntityItem)
    	{
            ...
            //Make wild pokemon level up naturally to their cap, to allow wild hatches
    ...
    public float getBlockPathWeight(IBlockAccess world, Vector3 location)
    {
    	...
    	if(getPokedexEntry().hatedMaterial!=null)
    	{
    		...
    		if(material.equalsIgnoreCase("water")&&block.getMaterial() == Material.water)
    		{
    			return 100;
    		}
    	}
	...
	{
		return sizes.set(getPokedexEntry().width, getPokedexEntry().height, getPokedexEntry().length).scalarMult(getSize());
	}
    ...
    public boolean attackEntityFrom(DamageSource source, float i)
    {
        ...
        if (getPokemonAIState(IPokemob.TAMED) && ((attacker instanceof EntityPlayer && ((EntityPlayer) attacker) == getOwner())))
        {
            return false;
        }
        ...
        {
            fleeingTick = 0;
            if(oldTarget!=null && getAttackTarget()!= oldTarget)
