...
public abstract class EntitySexedPokemob extends EntityStatsPokemob
{
    ...
    public void fuck(IPokemob male)
    {
    	if(this.getSexe()==MALE || male.getSexe() == FEMALE)
    	{
    		((EntityPokemob)male).fuck(this);
    		return;
    	}
    	int childPokedexNb = getChildSpecies(male);
        ...
        if (childPokedexNb > 0)
        {
            ...
            lay(childPokedexNb, male);
        }
    }

    public void lay(int pokedexNb, IPokemob male)
    {
    	//Adding some code that prevents egg laying it too many pokemobs nearby, this will prevent small caves willing with
    	//a very large number of geodude and zubat.
    	
    	if (PokecubeMod.debug)
    		System.out.println(this + " lay()");
        int i = (int)this.posX;
        int j = (int)this.posY;
        int k = (int)this.posZ;
    	...
    	List<EntityPokemob> near = worldObj.getEntitiesWithinAABB(EntityPokemob.class, new AxisAlignedBB(i-8, j-8, k-8, i+8, j+8, k+8));
    	
    	if(near.size()>= 5)
    	{
    		if (PokecubeMod.debug)
    			System.out.println("Too many pokemobs nearby, aborting Lay");
    		return;
    	}
    ...
    public Entity findLover()
    {
    	...
    	if (getSexe() == SEXLEGENDARY && !transforms)
    		return null;// legendaries cannot mate
    	
//        if (fleeingTick > 0 || getPokemonAIState(IPokemob.ANGRY))//TODO see if I need to re-implement fleeing tick somewhere
//        {
//            lover = null;
//            resetInLove();
//            return null;
//        }
        ...
        if((getSexe()==MALE  && !transforms) || males.size() > 0)
        {
        	return null;
        }
        ...
        if (inLove > 0)
        {
            ...
            for (int i = 0; i < list.size(); i++)
            {
                ...
                if(entityanimal == this || entityanimal.getPokemonAIState(IPokemob.TAMED) != this.getPokemonAIState(IPokemob.TAMED))
                	continue;
                ...
                if(!Vector3.isVisibleEntityFromEntity(this, entityanimal)|| entityanimal.getPokemonAIState(IPokemob.ANGRY))
                	continue;

                if (entityanimal != this && entityanimal.getHealth() > entityanimal.getMaxHealth()/1.5f)// && entityanimal.lover == null && entityanimal.inLove > 0)
                {
    ...
    @Override
    public int getChildSpecies(IPokemob male)
    {
		...
		for(String s: male.getMoves())
		{
			if(s!=null&&s.equalsIgnoreCase(IMoveNames.MOVE_TRANSFORM))
				otherTransforms = true;
		}
    	if(transforms && !otherTransforms && male.getTransformedTo() != this)
    	{
    		return male.getChildSpecies(male);
    	}
    	return getPokedexEntry().getChildNb(male.getPokedexNb());
    }
...
//	@Override
    public EntityAnimal spawnBabyAnimal(EntityAnimal entityanimal)
    {
        ...
        {
            lay(getChildSpecies((IPokemob) entityanimal), (IPokemob) entityanimal);
        }
    ...
    public boolean canMateWith(EntityAnimal entityAnimal)
    {
    	if(entityAnimal instanceof IPokemob)
    	{
    		...
    		if(transforms && otherTransforms)
    			return false;
    		else if(transforms || otherTransforms) //Anything else will mate with ditto
    			return true;
