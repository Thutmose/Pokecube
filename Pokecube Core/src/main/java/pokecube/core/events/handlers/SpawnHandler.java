...
public final class SpawnHandler
{
	...
	public static boolean isPointValidForSpawn(World world, Vector3 point, PokedexEntry dbe)
	{
		...
		if (entry.types[DAY] && !entry.types[NIGHT])
		{
            ...
            if(lightBlock == 0 && world.isDaytime())
                lightBlock = lightDay;
	...
	private int doSpawnForLocation(World world, Vector3 v)
	{
		...
		int b = t.getBiome(v);
		if(onlySubbiomes && b <= BiomeGenBase.getBiomeGenArray().length)
		{
			return ret;
		}
		...
		if (spawns.containsKey(b))
		{
			...
			if(dbe.getSpawnData().types[SpawnData.LEGENDARY])
			{
				...
				if(level < Mod_Pokecube_Helper.minLegendLevel)
				{
					return ret;
				}
			}
			...
			else
			{
				...
				if (dt > 50)
				{
					...
					System.err.println(t.getCentre() + " " + dt + " " + dbe + " " + num
							+ " Maximum was " + functions.get(-2) + " at " + maxtime);
				}
			}
		}
		return ret;
...
//		
//		System.out.println(water+" "+entry);
		...
		return down.canCreatureSpawn(worldObj, temp.getPos(), net.minecraft.entity.EntityLiving.SpawnPlacementType.ON_GROUND);// validSurfaces.contains(down);
	}
	...
	public static boolean canSpawn(TerrainSegment terrain, SpawnData data, Vector3 v, World world)
	{
		...
		if((data.getWeight(BiomeType.ALL.getType()))>0)
			return true;
