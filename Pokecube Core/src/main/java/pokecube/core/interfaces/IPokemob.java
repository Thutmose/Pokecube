...
public interface IPokemob extends IMoveConstants
{
    ...
    /**
     * Called when the mob spawns naturally.
     * Used to set held item for example.
     */
     ...
     * Evolve the pokemob.
     *
     * @param showAnimation true if we want to display the evolution animation
     ...
     * 
     * @param name of the entity this mob should evolve to
     */
	void setEvolution(String evolution);
	
	/**
	 * The evolution tick will be set when the mob evolves and then is decreased each tick.
	 * It is used to render a special effect.
	 * 
...
//     *
//     */
//    boolean getPokemonAIState(IPokemob.TAMED);
//    @Deprecated
//    /**
//     * from wolf code
//     *
//     */
//    void setPokemonTamed(boolean flag);
    ...
    /**
     * Displays a message in the console of the owner player (if this pokemob is tamed).
     * @param message
     */
    ...
    /**
     * Computes an attack strength from stats.
     * Only used against non-poke-mobs.
     *
    ...
    /**
     * Called when the level is up.
     * Should be overridden to handle level up events like evolution or move learning.
     * 
     ...
     * @param newlySpawned true if called by a spawner, false otherwise
     */
    ...
    /**
     * Will be called by the mother before she lays
     * to know what baby to put in the egg.
     *
     * @param male the male
    ...
    /**
     * The pokemob learns the specified move.
     * It will be set to an available position or erase an existing one if non are available.
     *
    ...
    /**
     * Called by attackEntity(Entity entity, float f).
     * Executes the move it's supposed to do according to his trainer command
     * or a random one if it's wild.
     * 
     ...
     * The set can fail because the mob is immune against this status
     * (a fire-type Pokemon can't be burned for example) or
     * because it already have a status. If so, the method returns false.
     * 
    ...
    /**
     * Sets the initial status timer. The timer will be decreased until 0.
    ...
    /**
     * Changes: {@link IMoveConstants#CHANGE_CONFUSED} for example.
     * The set can fail because the mob is immune against this change or
     * because it already has the change. If so, the method returns false.
     * 
    ...
    /**
     * Computes and returns an integer with protected properties (pokedexNb, stats, moves...).
     * 
	...
	/**
	 * This is called during move use to both the attacker and the attacked entity, in that order.
	 * This can be used to add in abilities, In EntityMovesPokemob, this is used for accounting for
	 * moves like curse, detect, protect, etc, moves which either have different effects per pokemon type, 
	 * or moves that prevent damage.
	 * @param move
	 */
    ...
    public static enum HappinessType
    {
    	...
    	public static void applyHappiness(IPokemob mob, HappinessType type)
    	{
    		...
    		if(type==BERRY && mob.getStatus() != STATUS_NON)
    		{
    			return;
    		}
    ...
    public static class MovePacket
    {
    	...
    	public MovePacket(IPokemob attacker, Entity attacked, String attack, PokeType type, int PWR, int criticalLevel, byte statusChange, byte changeAddition, boolean pre)
    	{
    ...
    public static class PokemobMoveStats
    {
        ...
        /**
         * Moves which have on-going effects, like leech seed, firespin, bind, etc
         */
