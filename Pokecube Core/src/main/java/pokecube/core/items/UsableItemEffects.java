...
public class UsableItemEffects
{
    ...
    public static void registerCapabilities(AttachCapabilitiesEvent<Item> event)
    {
        if (event.getCapabilities().containsKey(USABLE)) return;
        if (event.getObject() instanceof ItemBerry)
        {
            event.addCapability(USABLE, new BerryUsable());
        }
        if (event.getObject() instanceof ItemTM)
        {
            event.addCapability(USABLE, new TMUsable());
        }
        if (event.getObject() instanceof ItemVitamin)
        {
            event.addCapability(USABLE, new VitaminUsable());
        }
        if (event.getObject() == PokecubeItems.berryJuice)
        {
