...
public class RecipeRevive implements IRecipe {
	...
	public boolean matches(InventoryCrafting craftMatrix, World world) {
    	...
    	for(int i = 0; i<craftMatrix.getSizeInventory(); i++)
    	{
    		...
    		{
    			n++;
    			if(stack.hasTagCompound() && PokecubeManager.isFilled(stack))
    				other = stack;
    			if(stack.isItemEqual(PokecubeItems.getStack("Revive")))
    				revive = true;
    			if(stack.getItem() == PokecubeItems.getEmptyCube(-2))
    				seal = stack;
    		}
    	}
    	...
    	else if(revive)
		{
    		...
    		if(stack!=null && stack.hasTagCompound() && PokecubeManager.isFilled(stack))
    		{
    			...
    			if(stack.getItemDamage() == 32767)
    				PokecubeManager.setStatus(healed, IMoveConstants.STATUS_NON);
