...
public class MovesAdder implements IMoveConstants
{
    ...
    static void registerSpecialMoves()
    {
        ...
        registerMove(new Move_Basic(MOVE_BIDE)
        {
            ...
            public void postAttack(IPokemob attacker, Entity attacked, float f, int finalAttackStrength)
            {
                if (!((IPokemob) attacker).getMoveStats().biding)
                {
                    ((IPokemob) attacker).getMoveStats().SELFRAISECOUNTER = 100;
                    ((IPokemob) attacker).getMoveStats().biding = true;
                    ((IPokemob) attacker).getMoveStats().PHYSICALDAMAGETAKENCOUNTER = 0;
                    ((IPokemob) attacker).getMoveStats().SPECIALDAMAGETAKENCOUNTER = 0;
                }
                else
                {
                    if (((IPokemob) attacker).getMoveStats().SELFRAISECOUNTER == 0)
                    {
                        int damage = ((IPokemob) attacker).getMoveStats().PHYSICALDAMAGETAKENCOUNTER
        ...
        registerMove(new Move_Basic(MOVE_MIRRORCOAT)
        {
            ...
            public void postAttack(IPokemob attacker, Entity attacked, float f, int finalAttackStrength)
            {
                if (!((IPokemob) attacker).getMoveStats().biding)
                {
                    ((IPokemob) attacker).getMoveStats().SELFRAISECOUNTER = 30;
                    ((IPokemob) attacker).getMoveStats().biding = true;
                    ((IPokemob) attacker).getMoveStats().SPECIALDAMAGETAKENCOUNTER = 0;
                }
                else
        ...
        registerMove(new Move_Basic(MOVE_FOCUSENERGY)
        {
            ...
            public void postAttack(IPokemob attacker, Entity attacked, float f, int finalAttackStrength)
            {
                ((IPokemob) attacker).getMoveStats().SPECIALTYPE = IPokemob.TYPE_CRIT;
            }
        });
        ...
        registerMove(new Move_Basic(MOVE_ROLLOUT)
        {
            ...
            protected void finalAttack(IPokemob attacker, Entity attacked, float f)
            {
                if (doAttack(attacker, attacked, f))
                {
                    ...
                    double defCurl = ((IPokemob) attacker).getMoveStats().DEFENSECURLCOUNTER > 0 ? 2 : 1;
                    double rollOut = ((IPokemob) attacker).getMoveStats().ROLLOUTCOUNTER;
                    if (rollOut > 4)
                    {
                    ...
                    if (finalAttackStrength == 0)
                    {
                        ((IPokemob) attacker).getMoveStats().ROLLOUTCOUNTER = 0;
                    }
                    else((IPokemob) attacker).getMoveStats().ROLLOUTCOUNTER++;
                }
            }
        ...
        registerMove(new Move_Basic(MOVE_FURYCUTTER)
        {
            ...
            protected void finalAttack(IPokemob attacker, Entity attacked, float f)
            {
                if (doAttack(attacker, attacked, f))
                {
                    ...
                    if (finalAttackStrength == 0)
                    {
                        ((IPokemob) attacker).getMoveStats().FURYCUTTERCOUNTER = 0;
                    }
                    else((IPokemob) attacker).getMoveStats().FURYCUTTERCOUNTER++;
                }
            }
