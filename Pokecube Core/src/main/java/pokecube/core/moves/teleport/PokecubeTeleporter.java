...
public class PokecubeTeleporter extends Teleporter {
    ...
    {
        WorldServer world = FMLCommonHandler.instance().getMinecraftServerInstance().worldServerForDimension(entity.dimension);
        Vector3 v = Vector3.getNewVectorFromPool().set(world.getSpawnPoint()).addTo(0, entity.height, 0);
        
        if(!v.isClearOfBlocks(world))
        	 v.getTopBlockPos(world).add(1, entity.height);
