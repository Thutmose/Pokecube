...
public class Move_Basic extends Move_Base implements IMoveConstants
{
     ...
     * The attack category defines the way the mob will move in order to make its attack.
     *
     * @param name the English name of the attack, used as identifier and translation key
     * @param attackCategory can be either {@link MovesUtils#CATEGORY_CONTACT} or 
	...
	public void attack(IPokemob attacker, Vector3 location, float f)
    {
    	...
    	{
    		List subTargets = new ArrayList<Entity>();
    		if(subTargets.contains(attacker))
    			subTargets.remove(attacker);
        ...
        if ((getAttackCategory() & CATEGORY_SELF) == 0)
        {
        	...
        	if(n>0)
        	{
	        	...
	        	{
	        		if(e!=null)
	        			finalAttack(attacker, e, finalAttackStrength);
	        	}
        	}
        	else
        	{
	...
	protected void finalAttack(IPokemob attacker, Entity attacked, float f, boolean message){
    	if (doAttack(attacker, attacked, f)) {
            ...
            {
        		if (sound!=null){
            		((Entity) attacker).worldObj.playSoundAtEntity((Entity) attacker, sound, 0.5F, 0.4F / (MovesUtils.rand.nextFloat() * 0.4F + 0.8F));
        		}
        		List<EntityLivingBase> hit = MovesUtils.targetsHit(((Entity) attacker), v.set(attacked).addTo(0, attacked.height/3, 0));
        		...
        		for(Entity e: hit)
        		{
        			...
        			int pwr;
        	        int finalAttackStrength = MovesUtils.attack(attacker, attacked, name, move.type, pwr = getPWR(attacker, attacked), move.crit, statusChange, changeAddition);
            ...
            else if((getAttackCategory() & CATEGORY_SELF) == 0)
            {
            	...
            	{
            		attacked = MovesUtils.targetHit(((Entity) attacker), v.set(attacked).addTo(0, attacked.height/3, 0));
            	}
        		if (sound!=null){
            		((Entity) attacker).worldObj.playSoundAtEntity((Entity) attacker, sound, 0.5F, 0.4F / (MovesUtils.rand.nextFloat() * 0.4F + 0.8F));
        		}
    	        ...
    	        int finalAttackStrength = MovesUtils.attack(attacker, attacked, name, move.type, getPWR(attacker, attacked), move.crit, statusChange, changeAddition);
	...
	public void doSelfAttack(IPokemob mob, float f)
    {
    	if(doAttack(mob, (Entity) mob, f))
    	{
			...
			if (sound!=null){
	    		((Entity) mob).worldObj.playSoundAtEntity((Entity) mob, sound, 0.5F, 1F / (MovesUtils.rand.nextFloat() * 0.4F + 0.8F));
			}
     ...
     * @param attacked
     * @param f
     * @param finalAttackStrength the number of HPs the attack takes from target
     */
    @Override
	...
	public void notifyClient(Entity attacker, Vector3 attacked, Entity target)
    {
        if (!mod_Pokecube.isOnClientSide())
        {
            ...
            try
            {
	            ...
	            PokecubeClientPacket packet = PokecubePacketHandler.makeClientPacket(PokecubePacketHandler.CHANNEL_ID_PokemobMove, toSend.getBytes());
    ...
    {
    	if(attacked == null && getAttackCategory()!=CATEGORY_SELF)
    		return false;
    	...
    	else if(this.hasStatModTarget&&f==0)
    		return ((IPokemob)attacker).getMoveStats().TARGETLOWERCOUNTER==0;
