...
public class Move_Utility extends Move_Basic {
	...
	public void attack(IPokemob attacker, Entity attacked, float f)
    {
		...
		if( (attacker instanceof IPokemob&&(PokecubeMod.semiHardMode||PokecubeMod.debug) && attacker.getPokemonAIState(IPokemob.TAMED)))
		{
			...
			{
				((EntityPlayer) owner).addChatMessage(new ChatComponentText("Your pokemon needs to eat more Berries to do that"));
			}
		}
    }
    ...
    {
    	if(!(PokecubeMod.semiHardMode||PokecubeMod.debug))
    		return;
    	
		boolean used = false;
		boolean repel = SpawnHandler.checkNoSpawnerInArea(((Entity)user).worldObj, location.intX(), location.intY(), location.intZ());
		...
		if(owner!=null && owner instanceof EntityPlayer)
		{
			...
			if(!repel)
			{
				((EntityPlayer) owner).addChatMessage(new ChatComponentText("Your pokemon is too disgusted by the repel to do that"));
				return;
			}
    		EntityPlayer player = (EntityPlayer) owner;
    		
    		BreakEvent evt = new BreakEvent(player.worldObj, location.getPos(), location.getBlockState(player.worldObj), player);
