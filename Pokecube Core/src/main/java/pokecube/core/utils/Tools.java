...
//		return PokecubeMod.MAX_DAMAGE-((value<<10) + uid);
        return (int) (PokecubeMod.MAX_DAMAGE - value);
    }

    public static int getHealth(int maxHealth, int serialization)
    {
//		int health = (int) (((PokecubeMod.MAX_DAMAGE-serialization)>>10)*maxHealth/PokecubeMod.FULL_HEALTH);
...
//	public static int getPokedexNumber(int serialization){
//		return (PokecubeMod.MAX_DAMAGE-serialization)&PokecubeMod.UID_MASK;
//	}

    public static int getHealedPokemobSerialization()
    {
//		return serialize(1, 1, pokedexNumber);
        return PokecubeMod.MAX_DAMAGE - PokecubeMod.FULL_HEALTH;
    }

    //ADDED + 128 for java byte nonsense
    ...
    public static int getStat(int BS, int IV, int EV, int level, int mod, int nature)
    {
        ...
        return Math.max((int)((((MathHelper.floor_double((2 * BS) + IV + EP)  * level / 100) + 5)*modifier*natModifier)),5);
    }
    ...
    public static int[] getStats(IPokemob mob)
    {
    	...
    	ret[1] = getStat(mob.getBaseStats()[1], mob.getIVs()[1], mob.getEVs()[1], mob.getLevel(), mob.getModifiers()[1], mob.getNatureModifiers()[1]);
    	ret[2] = getStat(mob.getBaseStats()[2], mob.getIVs()[2], mob.getEVs()[2], mob.getLevel(), mob.getModifiers()[2], mob.getNatureModifiers()[2]);
    	ret[3] = getStat(mob.getBaseStats()[3], mob.getIVs()[3], mob.getEVs()[3], mob.getLevel(), mob.getModifiers()[3], mob.getNatureModifiers()[3]);
    	ret[4] = getStat(mob.getBaseStats()[4], mob.getIVs()[4], mob.getEVs()[4], mob.getLevel(), mob.getModifiers()[4], mob.getNatureModifiers()[4]);
    	ret[5] = getStat(mob.getBaseStats()[5], mob.getIVs()[5], mob.getEVs()[5], mob.getLevel(), mob.getModifiers()[5], mob.getNatureModifiers()[5]);
   // 	mob.setStats(ret);
    	return ret;
    }
    ...
    public static int[] getStatsUnMod(IPokemob mob)
    {
    	...
    	ret[1] = getStat(mob.getPokedexEntry().getStatATT(), mob.getIVs()[1], mob.getEVs()[1], mob.getLevel(), mob.getNatureModifiers()[1]);
    	ret[2] = getStat(mob.getPokedexEntry().getStatDEF(), mob.getIVs()[2], mob.getEVs()[2], mob.getLevel(), mob.getNatureModifiers()[2]);
    	ret[3] = getStat(mob.getPokedexEntry().getStatATTSPE(), mob.getIVs()[3], mob.getEVs()[3], mob.getLevel(), mob.getNatureModifiers()[3]);
    	ret[4] = getStat(mob.getPokedexEntry().getStatDEFSPE(), mob.getIVs()[4], mob.getEVs()[4], mob.getLevel(), mob.getNatureModifiers()[4]);
    	ret[5] = getStat(mob.getPokedexEntry().getStatVIT(), mob.getIVs()[5], mob.getEVs()[5], mob.getLevel(), mob.getNatureModifiers()[5]);
   ...
   // 	mob.setStats(ret);
    ...
    public static double modifierToRatio(byte mod, boolean accuracy)
    {
        ...
        if(mod==0)
        	...
        	modifier = !accuracy?1.5:4/3d;
        else if(mod==2)
        	modifier = !accuracy?2:5/3d;
        else  if(mod==3)
        	modifier = !accuracy?2.5:2;
        else if(mod==4)
        	modifier = !accuracy?3:7/3d;
        else if(mod==5)
        	modifier = !accuracy?3.5:8/3d;
        else if(mod==6)
        	modifier = !accuracy?4:3;
        else if(mod==-1)
			modifier = !accuracy?2/3d:3/4d;
        else if(mod==-2)
			modifier = !accuracy?1/2d:3/5d;
        else if(mod==-3)
			modifier = !accuracy?2/5d:3/6d;
        else if(mod==-4)
			modifier = !accuracy?1/3d:3/7d;
        else if(mod==-5)
			modifier = !accuracy?2/7d:3/8d;
        else if(mod==-6)
			modifier = !accuracy?1/4d:3/9d;
		return modifier;
    }
    ...
    public static int levelToXp(int type, int level)
    {
        switch (type)
        {
            ...
            case 2:// 1 059 860
                return MathHelper.floor_double(1.05D * Math.pow(level, 3)) + 1; // it should be the parabollic
    ...
    //I am lazy, don't want to do the maths, so am making tables for the erratic and fluctuating groups.
    ...
    public static int xpToLevel(int type, int exp)
    {
        ...
        switch (type)
        {
            ...
            case 2:
                level = MathHelper.floor_double(Math.pow(exp / 1.05D, 0.333333333333333333333F)); // it should be the parabollic
                break;
    ...
    {
        float multiplier = (float) (1.5F / Math.sqrt(PokecubeMod.MAX_DENSITY)); // to make the evolution more rapid and the game less boring. But it doesn't come from pokemon's rules
        //EDIT: finally restored the multiplier.
    ...
    {
        double cubeBonus = 0;//PokecubeMod.getCubeBonus(pokecubeId, pokemob);
    ...
    {
    	int ret = 0;
    	List<EntityLiving> list = world.getEntitiesWithinAABB(EntityLiving.class, location.getAABB().expand(distance, distance, distance));
    	for(Object o: list)
    	{
