	...
	private void placeCropAt(World world, BlockPos pos, String berryName){
	   ...
	   public boolean generateTree(World par1World, Random par2Random, BlockPos pos, IBlockState wood, IBlockState leaves)
	    {
	        ...
	        if (y >= 1 && y + l + 1 <= 256)
	        {
	                ...
	                for (int l1 = x - b0; l1 <= x + b0 && flag; ++l1)
	                {
	                    for (j1 = z - b0; j1 <= z + b0 && flag; ++j1)
	                    {
	                        if (i1 >= 0 && i1 < 256)
	                        {
	                            ...
	                            if (!par1World.isAirBlock(temp) &&
	                                !block.isLeaves(par1World, temp) &&
	                                 block != Blocks.grass &&
	                                 block != Blocks.dirt &&
	                                !block.isWood(par1World, temp))
	                            {
	                                flag = false;
	                            }
	                        }
	            ...
	            else
	            {
	                ...
	                boolean isSoil = true;//(soil != null && soil.canSustainPlant(par1World, par3, par4 - 1, par5, EnumFacing.UP, (BlockSapling)Block.sapling));
	   ...
	   public boolean generatePalmTree(World world, Random par2Random, BlockPos pos, IBlockState wood, IBlockState leaves)
	   {
	       ...
	       if (pos.getY() >= 1 && pos.getY() + l + 1 <= 256)
	       {
	    	   ...
	    	   for(int i = 1; i<l; i++)
	    	   {
	    		   if(world.isAirBlock(temp = pos.up(i)))
	    	   ...
	    	   if(!stopped)
	    	   {
	    		   ...
	    		   for(int i = 0; i<=l/1.5; i++)
	    		   {
	        		   ...
	        		   if(world.isAirBlock(temp))
	        			   world.setBlockState(temp, leaves);
	        		   else if(i!=0)
	        		   {
	        			   stopped = true;
	        			   break;
