...
public abstract class ComponentVillageBase extends House1
{
	...
	protected ComponentVillageBase(Start startPiece, int orient, Random par3Random, StructureBoundingBox par4StructureBoundingBox, EnumFacing par5)
	{
    ...
    protected void fillWithMetaBlocks(World par1World, StructureBoundingBox par2StructureBoundingBox, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, Block placeBlockID,
            int placeBlockMeta, boolean alwaysReplace)
    {
         IBlockState oldState = placeBlockID.getStateFromMeta(placeBlockMeta);
         IBlockState newState = super.func_175847_a(oldState);
         
         super.fillWithBlocks(par1World, par2StructureBoundingBox, minY, minY, minZ, maxX, maxY, maxZ, newState, newState, alwaysReplace);
         
       //  super.fillWithMetadataBlocks(par1World, par2StructureBoundingBox, minX, minY, minZ, maxX, maxY, maxZ, i2, j2, k2, l2, alwaysReplace);
    	//super.fillWithMetadataBlocks(par1World, par2StructureBoundingBox, minX, minY, minZ, maxX, maxY, maxZ, placeBlockID, placeBlockMeta, placeBlockID, placeBlockMeta, alwaysReplace);
     }
	...
	protected void placeTorch(World world, StructureBoundingBox structBB,
			int posX, int posY, int posZ, EnumFacing dir)
	{
...
//            world.spawnEntityInWorld(entityvillager); //TODO villager spawn
      ...
      for (int k = this.boundingBox.minZ; k <= this.boundingBox.maxZ; k++) {
        for (int l = this.boundingBox.minX; l <= this.boundingBox.maxX; l++) {
          if (par2StructureBoundingBox.isVecInside(new Vec3i(l, 64, k)))
          {
            ...
            i += Math.max(par1World.getTopSolidOrLiquidBlock(pos).getY(), par1World.provider.getAverageGroundLevel());
            j++;
          }
        }
      }
     ...
     */
    protected void fillCurrentPositionBlocksDownwards(World par1World, Block par2, int par3, int par4, int par5, int par6, StructureBoundingBox par7StructureBoundingBox)
    {
        int j1 = this.getXWithOffset(par4, par6);
        int k1 = this.getYWithOffset(par5);
        int l1 = this.getZWithOffset(par4, par6);
